...
const newRoom = {
  ...
  description: "A large room for two to four people. One premium king size bed, premium couch bed, premium tv, and mini fridge. Premium furniture, bedding, bathroom and views.",
  maxOccupants: 4,
  beds: 2
};
...
test('room updated', async () => {
  const body = {"description": "A massive room for two to six people. Two single king size beds, couch bed, premium tv, and mini fridge.", "maxOccupants": 6, "beds": 3}
...
test('room deleted', async () => {
  const body = {}
  const response = await api.delete('/rooms/62910b0acb70612be56c67a2').send(body);
  expect(response.status).toBe(204);
